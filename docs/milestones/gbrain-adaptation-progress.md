...
| 文件 | 用途 |
|---|---|
| `docs/product/gbrain-feature-inventory.md` | GBrain 原生功能盘点矩阵：判断能力应直接沿用、adapter 包装、Skillify 补齐还是暂不接入。 |
| `docs/specs/gbrain-ingest-workflow.md` | 原始资料进入 GBrain source 的导入、提炼、审核和索引流程。 |
| `docs/operations/gbrain-agent-citation-fixer-runbook.md` | GBrain agent / citation-fixer 的 OAuth 绑定、worker 模式、DeepSeek gateway loop 和预检流程。 |
| `docs/adr/0003-gbrain-service-adapter.md` | 决定 Project_R 通过 GBrain HTTP/MCP service-account adapter 调用 GBrain。 |
| `docs/adr/0004-gbrain-company-wiki-source-path.md` | 早期 MVP 决定 `derived/` 直接作为 `company-wiki` source repo；已被 ADR 0019 取代。 |
| `docs/adr/0005-gbrain-derived-local-git.md` | 早期 MVP 决定 `derived/` 使用本地 Git 做审计、对比和回滚；后续审计目标应迁移到 `gbrain-ready/` 与 manifests。 |
...
| `docs/adr/0010-source-scoped-knowledge-ingest-review-policy.md` | 决定知识录入按来源空间分责：管理员公司知识免二次审核，私人空间不入公司库，项目/客户录入按当前工作区权限和当前路径范围进入对应 source 并通知触发用户。 |
...
| `docs/adr/0019-gbrain-ready-preprocessing-source-repos.md` | 决定用户源文件目录不再创建 `derived/`，GBrain source repo 改为 `_preprocessed/.../gbrain-ready/`。 |
| `docs/adr/0020-project-r-gbrain-native-ui-shells.md` | 决定客户情报、项目洞察等 UI 是 Project_R 对 GBrain 原生能力的转发和可视化，不重做 GBrain 知识内核。 |
...
-> 按 source scope 写入 GBrain-ready Markdown source repo
...
2026-06-20 GBrain `0.42.51.0` 上游升级已完成 Stage 8 本地切换：`reference/gbrain-master` 已替换为验证过的 0.42.51 candidate，旧源码备份为 `reference/gbrain-master.pre-0.42.51-20260620-124345`，旧 runtime 数据备份为 `backend/workspace_data/_gbrain_backup_pre_0.42.51_20260620-124345`。本轮新增 `patches/gbrain/0.42.51/0008-doctor-resolver-health-windows-crlf.patch`，修复 Windows CRLF skill frontmatter 解析导致的 resolver false failure，并补上 `skill-optimizer` resolver row；全面检测后追加 `0009-windows-source-run-apply-migrations.patch`，修复 Windows/source-run `apply-migrations` 依赖全局 `gbrain` 的兼容缺口。切换后 `/health` 返回 `version=0.42.51.0`、`engine=pglite`，正式 `_gbrain` schema 已从 107 迁移到 119；`doctor --fast --json` 为 warnings-only，`resolver_health=ok`、`brain_checks_score=100`，仅剩 `retrieval_reflex_health` host integration warning。详见 `docs/validation/gbrain-upgrade-0.42.51-cutover.md`。
...
| PDF 摄取 | 已完成 MVP，路由规则已收紧 | 已接入 MiMo 视觉辅助结构化提炼；2026-06-04 已确认 PDF 统一使用 MiMo V2.5，文本抽取只作为辅助证据，最终写入 GBrain-ready Markdown 后同步对应 source。仍未完成逐条条款回链、图表全量 OCR/视觉校验、章节级拆页策略。 |
...
| 来源展示 | 已完成基础版，目标架构待迁移 | 前端可展示 GBrain 来源标签，并在右侧来源面板渲染 Markdown 片段；项目 source 结果会尽量补当前实现的 `derived_file`、`source_file`、`source_line`、`source_page` 和定位文本。迁移后普通用户只能预览安全源文件和引用片段，不直接暴露 `_preprocessed/` 过程目录。 |
...
| 知识审核 | 来源规则已落地 MVP，录入范围规则待迁移 | 管理员后台录入公司知识不需要额外审核；用户私人空间附件不进入公司库；项目/客户录入必须按当前工作区权限和路径范围执行。2026-06-04 已确认项目默认录入当前打开路径并递归处理子文件夹，需二次确认；右键可录入单文件；审核队列保留给答案反馈纠错、显式提升公司知识和异常情况。 |
...
| graph / timeline / entity merge | 已完成客户情报后端/API/回归、管理员安全操作竖切片、工作区内客户画像/项目事件入口、工作区级实体候选审核 MVP、token-bound native graph/timeline/backlinks context MVP，工作区侧栏筛选/节点详情/Timeline 分组折叠与密度切换/小画布/事件详情/大画布基础 pan/zoom/source preview MVP 已接入 | 2026-06-02 新增 `core/gbrain_graph.py`，从 source-of-record `derived/` Markdown/frontmatter 生成 `nodes/edges/events/citation`，当前正式验收支持 `company-wiki`、受限客户情报数据范围和 `project-*` 路径；`customer-reference` 只是当前早期实现 source id。客户工作区图谱读取客户情报 GBrain 数据，项目工作区图谱读取当前 `project-*` source。管理员和工作区实体候选审核、别名审核沉淀、安全 diff 预览、受控 frontmatter 引用改写、token-bound native graph/timeline/backlinks context、侧栏筛选/节点详情、Timeline 分组折叠/密度切换、小画布、事件详情、大画布 pan/zoom 和安全 source preview 已接入；普通成员不能处理实体候选，未授权客户用户 403。自动布局优化、native timeline 可视化增强、重复页面自动合并、正文事实合并和实体页删除仍未完成。 |
| schema pack | 未定制 | 当前使用默认 `gbrain-base-v2`；后续需要评估是否为项目资料、会议、标准、合同等设计 Project_R schema pack。 |
| Project_R extractor classifier / skills | 项目主链路 MVP 已完成，仍需 Skill 化 | 已实现 `core/extractor_classifier.py`，普通用户不选择 API Key；Project_R 后端先按 `source_scope`、`file_kind`、`extraction_complexity` 自动选择 `extractor_profile/model_profile`。文字/Markdown/DOCX 走 DeepSeek，PDF/图纸/图片/截图走 MiMo V2.5，MP4 自动/长视频先转写，EML 邮件线程和 EML 附件递归已能进入项目 source。批量邮件线程合并、置信度、绝对时间戳回链、区域级图片引用和可重复 Project_R Agent / Skills 封装仍未完成。GBrain Skillify 只保留为后 Markdown 阶段或设计参考。 |
...
- Customer Intelligence Source：新增 `backend/core/gbrain_customer_sources.py`、`backend/scripts/gbrain_customer_reference_ingest.py`、`backend/scripts/gbrain_customer_reference_regression.py` 和 `backend/tests/fixtures/gbrain_customer_reference_regression_cases.json`。早期统一客户情报 source id `customer-reference` full sync 成功，三条真实客户问题 `5Points`、`18 Mary Avenue`、`Aaron Morris` 均首位命中正确画像页；后续产品文档和 UI 应使用“客户情报源 / 客户画像数据 / Customer Intelligence Source”术语。
...
- Self-Wiring Knowledge Graph / Entity Enrichment：资料已按公司、项目、客户情报数据范围进入 GBrain；客户画像读取 GBrain 已精炼吸收的客户情报数据，项目资料使用 `project-*`。frontmatter 中保留 `linked_people`、`linked_projects`、`linked_companies` 等关系字段。本轮新增 Project_R graph adapter，直接从 GBrain source-of-record Markdown 生成业务图谱，并保留未建档实体为 `unresolved_entity`；`backend/scripts/gbrain_graph_regression.py` 已覆盖 5Points、18 Mary Avenue、Aaron Morris。工作区级客户画像/项目事件入口、实体候选审核、token-bound native graph/timeline/backlinks context、图谱筛选、节点详情、Timeline 分组折叠/密度切换、小画布、事件详情和全屏大画布基础 pan/zoom MVP 已接入。尚未完成自动布局优化、native timeline 可视化增强、重复页面自动合并、正文事实合并和实体页删除。
- Multi-Source Ingestion：当前正式验收口径为三类 source scope：`company-wiki`、项目 `project-*`、受限客户情报数据范围。客户图片/ZIP/Excel 等复杂资料不直接污染 GBrain，先进入 pending extractor manifest；客户复杂资料 extractor 已按用户确认移出当前 goal，等待真实样本后另开目标。
...
2. 扩展项目 extractor 能力：PDF/建筑图纸的 MiMo V2.5 视觉提炼、EML 邮件线程解析、EML 附件递归、图片/截图提炼、MP4 自动分段转写和说话人/术语纠错已完成项目 MVP；下一步补人工抽检、质量回归集、区域级引用定位和文件预览 UI。
...
6. 固化 Project_R extractor skills：统一分类器和项目主链路已实现；下一步把项目复盘、资料提炼、PDF、图纸、音频/视频、会议沉淀、图片/截图和邮件整理封装成可重跑、可配置、可观测的 Project_R Agent / Skills；GBrain 继续负责 Markdown 入库后的 query/think/citation/maintain/graph 等流程。
7. 设计客户情报 / 客户画像模块：基于 `workspace_data/customer/` 与受限客户情报 GBrain 数据范围，客户工作区权限入口和客户画像只读入口已完成 MVP；客户复杂资料 extractor 暂时不纳入当前 goal。下一步补 GBrain native timeline/graph 合并、People Graph / Company Graph / Project Graph 的自动提炼增强，以及客户/项目/company 临时联合查询边界。
...
- 新增 `backend/core/gbrain_project_ingest.py`：当前 MVP 扫描 Project_R 项目工作区根目录，排除 `derived/`、`manifests/`、`.trash/`、`.git/` 和 PDF 同名图片侧车目录。2026-06-04 目标行为改为处理文件面板当前打开路径，递归子文件夹前二次确认；右键可录入单文件。
- 项目目录到 GBrain-ready 分类的第一版映射：`01-合同与报价 -> contracts`、`02-图纸与技术资料 -> technical`、`03-会议纪要 -> meetings`、`04-变更与签证 -> changes`、`05-生产与发货 -> production`、`06-现场与客诉 -> site`、`99-未归档文件 -> unfiled`。
...
- PDF 录入由 classifier 先判定文件类型并统一路由到 MiMo V2.5；本地文本抽取只能作为辅助证据，不能直接成为 GBrain 正文。建筑图纸、截图、设计图片和视觉版式资料同样走 MiMo V2.5，不使用 MiMo V2.5 Pro。
- 2026-05-31 自动路由规则已落地并扩展：新增 `core/extractor_classifier.py`，项目文件录入前输出 `source_scope`、`file_kind`、`extraction_complexity`、`extractor_profile` 和 `classifier_reason`。普通用户不选择 API Key；后端自动把 Markdown/TXT/DOCX 路由到 DeepSeek 文字路线，把 PDF/图纸/图片/截图路由到 MiMo V2.5 视觉路线，把音视频路由到转写流程，把 EML 路由到邮件线程提炼并递归处理附件。
- 当前 MVP 的工作区录入接口已接入项目编译结果：`POST /workspaces/{workspace_id}/knowledge/ingest` 先编译项目文件并写 `manifests/project-source-ingest-manifest.json`，再确保 GBrain 项目 source 注册并同步；正式 compiled 文件只有在 sync 成功后才标记 `rag_status=indexed`。上传文件和会话附件保存不再误标 `indexed`。异步队列由 `WorkspaceIngestJob` 记录 `queued/running/succeeded/failed`、结果 JSON 和失败原因。
- 项目 pending review 不再作为默认路径；项目文件由用户在项目文件面板点击“录入”后，按当前打开路径递归录入知识目标范围内且尚未入库的文件；已具备 preprocessor Skill 的文件写入对应 `gbrain-ready/` 并同步 project source，不进入公司库、不创建公司知识审核项；暂未具备 preprocessor 的文件保留为 `pending_capability` 或明确失败状态；完成或失败后通知点击录入的用户。
- 前端项目文件面板应显示录入数量、递归确认、排队/执行中状态、完成/失败提示，并展示 `synced / gbrain_ready / source_changed / needs_repreprocess / pending_capability / ignored / failed` 等文件状态。
- 新增和改写测试已覆盖项目 Markdown/txt、DOCX、PDF/图纸 MiMo 提炼、图片/截图 MiMo 提炼、媒体自动转写、长视频抽音频后越过原视频体积限制、说话人/术语纠错元数据、EML 邮件提炼、EML 附件递归、项目 source sync、项目不创建管理员审核项、异步 job 和通知触发用户；后续应迁移到独立 preprocessor Skills 和 `_preprocessed/.../gbrain-ready/` 路径。
...
- 查询隔离与定位验证：项目工作区组合 `/query` 中的当前项目 source 能命中 synthetic 独特事实 `PR_GBRAIN_PROJECT_SOURCE_SYNTHETIC_FACT_20260530` 和 `SYN-QUOTE-GRAIN-001`，也能命中 Lucerna 邮件、20260529 会议行动项和 GENERAL ARRANGEMENT 图纸页；每条项目来源尽量返回 `derived_file`、`source_file`、`source_line` 和 `source_page`，并在来源内容末尾附加定位块。`company-wiki` 同 synthetic 问题不包含该项目事实。
...
未闭环：该验收说明项目 source 录入、PDF/图纸视觉提炼、图片/截图视觉提炼、MP4 自动转写、长视频分段、说话人/术语纠错、EML 邮件线程提炼、EML 附件递归、项目内引用定位和异步队列均完成 MVP；但模型提炼质量仍需人工抽检和回归集扩展。尤其长视频当前分段时间戳仍以段内时间为主，尚未做全局绝对时间轴合并；说话人识别依赖模型推断，不等同于专业 diarization；图片提炼仍需补区域级引用定位。文件预览 UI 尚未完成。
...
1. 公司全局 source 与至少一个项目 source 都能完成闭环：管理员公司知识 `source files -> gbrain-ready -> sync -> query/think -> citation`；项目知识“项目文件面板当前路径录入 -> project gbrain-ready -> sync -> 项目内 query/think -> citation -> 通知触发用户”。
2. `/query` 不只是返回 chunk，而是能够稳定使用 GBrain query/think、引用、gap/conflict 信息。
3. PDF、图纸、会议转写、音视频、图片/截图、邮件至少都有明确的 Project_R extractor 流程、失败状态、pending capability 状态和按 source scope 入库策略。
