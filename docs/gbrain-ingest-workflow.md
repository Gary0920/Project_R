...
backend/workspace_data/customer/{客户工作区}/
  01-客户档案/
  02-联系人与关系/
  03-沟通记录/
  04-原始资料/
...
└── customer/
    └── {workspace_id}-{customer_slug}/
        ├── gbrain-ready/   # 当前客户情报 source repo
        ├── runs/
        └── manifests/
```
...
- 客户情报的目标是把 `workspace_data/customer/` 中的资料经 Project_R 提炼后投喂给 GBrain，让 GBrain Entity Enrichment 自动建立 People Graph、Company Graph 和 Project Graph，服务销售团队做客户关系管理、关键人判断、公司关系判断和项目关系判断。
...
- 客户工作区可提供“客户情报”入口，标签包括画像概览、图谱、时间线、实体处理和 GBrain 状态。该入口只是 Project_R 对 GBrain 原生能力的权限、转发和展示壳，不是 Project_R 自建客户画像系统。
...
- 当前代码 MVP 中，`company-wiki` source 已注册到 `backend/workspace_data/global/company-wiki/derived/`；2026-06-04 目标架构改为注册到 `backend/workspace_data/_preprocessed/company/company-wiki/gbrain-ready/`。
