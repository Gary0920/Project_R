...
| 项目知识库 | 同一 brain 多 source，天然适合项目隔离。 | `source_id` 多源导入、查询 scope；`.gbrain-source` 路由。 | 每个工作区/项目映射独立 project source；第一版 adapter 已使用 `project-{brand}-{workspace_id}` 稳定 source id、`derived/` 路径、显式 query scope 和项目文件编译 manifest。新规则要求项目文件面板一键录入当前项目所有未入库文件；可处理文件直接同步项目 source，缺 extractor 的音频/视频、图片/截图、邮件、复杂 PDF 标记为 `pending_extractor_capability`；不进入公司库、不走管理员审核。2026-06-01 已补项目 source 专属 Think OAuth client manifest，项目一键录入 sync 成功后会准备 token-bound `/query` 能力。 | Adapter 包装 | Project_R 必须在查询时强制带 source scope，避免跨项目泄露；等待 Gary 新建测试项目后重新跑真实 one-click ingest/sync/query/think 回归。 |
| 客户情报 / 客户画像 source | 同一 brain 的统一受限 source 可承载客户业务情报；GBrain graph/timeline/think 适合关系网络、人物画像和沟通脉络。 | `customer-reference` source scope、OAuth source scope、graph/timeline、citations、Project_R 工作区权限。 | 已确认客户画像是“受限业务情报 source”，不是公司公共知识库；不得写入 `company-wiki`。2026-06-02 最新决策：客户知识库只有一个统一 source `customer-reference`，所有客户信息在同一库内共存，不再按每个客户拆独立 GBrain source。Project_R `customer` 工作区负责权限入口：系统管理员可新建客户情报工作区，默认隐藏，普通用户必须由成员或组别授权后才能搜索/进入。`/query` 在客户工作区中显式路由到 `customer-reference`，不得回落到 `company-wiki`；本轮 Think 回归加入 5Points、18 Mary Avenue、Aaron Morris，验证同一客户库内查询特定客户不会混入其他客户答案或 citation。图片/ZIP/Excel 等客户复杂资料只保留 pending 状态，不污染 source。 | Adapter 包装已起步 / 待产品化 | 仅营销组或授权成员可查；不能污染公司规章和行业知识；当前 goal 不要求完成客户复杂资料 extractor；native timeline 可视化增强、正文级实体合并/删除审核和项目/客户/公司临时联合查询规则仍需单独设计。 |
