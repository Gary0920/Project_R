...
def generated_file_payload(file_id: str, filename: str, mime_type: str) -> dict[str, str]:
    return {
        "id": file_id,
        "filename": filename,
        "mime_type": mime_type,
...
def create_generated_file(
    ...
    *,
    ...
    render_document(format_spec.key, title, content, output_path)
    ...
    db.flush()
    return generated_file_payload(file_id, filename, format_spec.mime_type)
