...
class GBrainIngestTests(unittest.TestCase):
    ...
    def test_compiles_docx_to_meeting_markdown(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(frontmatter["source_file_type"], "docx")
    ...
    def test_audio_with_transcript_sidecar_goes_to_pending_review_meeting(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(frontmatter["source_file_type"], "mp3")
            self.assertEqual(frontmatter["prompt_version"], "rules-meeting-audio-video-v1")
            self.assertEqual(frontmatter["preprocess_status"], "partial")
    ...
    def test_compiles_pdf_with_structured_extractor_when_enabled(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(frontmatter["review_status"], "pending_review")
            self.assertEqual(frontmatter["preprocess_skill"], "pdf-structured-preprocess")
