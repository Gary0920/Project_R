...
- 用户的工作流为vibecoding,本职工作并不是IT相关,不懂代码编程及软件开发,你需要用简单的语言解释方案与用语.在用户提出不合理方案,或与先前设计框架文件内容信息不一致方案时,必须提醒用户二次确认.
- 搜索所有相关代码,是否已有可复用功能/代码
- 识别问题根因
- 发现架构问题,不要继续的在单一的文件中,重复堆叠功能代码,应按需/按功能对代码进行结构化设计.如原生文件已经大量堆叠代码,导致文件代码结构臃肿,对后续的优化/开发/维护造成风险,请在本次方案中针对的,安全的拆分臃肿代码.
- 如果有不清楚的，请向我收集必要的信息
