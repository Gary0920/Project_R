...
- 用户源文件目录只保存用户上传或管理员投放的原始文件；不得在 `workspace_data/project/{品牌}/{项目代号}`、`workspace_data/customer/{slug}` 等用户源文件目录下新增 `derived/`。
...
- 工作区在软件使用层面是项目资料容器；业务上每个项目工作区一一对应一个项目，通常以项目代号命名。代码内部继续使用 `workspace` 历史命名。公司项目资料位于 `backend/workspace_data/project/{品牌}/{项目代号}`，品牌固定为 `AURA`、`BFI`、`SPECWISE`、`SYNOVA`；用户默认入口显示为 `{用户名}的工作台`，可见置顶且不可删除、不可重命名、不可归档，只对本人可见。个人工作台不是个人文件库、不是个人知识库、不是个人记忆系统、不是 GBrain source，也不再提供右侧个人文件面板；用户个性和常用表达优先通过用户自定义提示词、会话级提示词和本轮显式附件/引用表达。当前代码如仍使用 `backend/workspace_data/user/{用户名}`，只视为会话材料暂存/兼容路径，不代表长期知识源，后续可在替代能力闭环后退役。第一版只有系统管理员可新建项目工作区；公司项目默认开放给所有用户，成员列表不代表全部访问者，只用于项目管理员、隐藏项目人员白名单和需要显式通知/治理的协作人；隐藏项目才按成员白名单和组别白名单限制搜索/进入。公司项目默认目录为 `01-合同与报价`、`02-图纸与技术资料`、`03-会议纪要`、`04-变更与签证`、`05-生产与发货`、`06-现场与客诉`、`99-未归档文件`。Agent 模式默认显示当前项目工作区文件目录/上下文面板，供用户检查参考资料是否正确、是否遗漏；Chat 模式只提供轻量入口。当前已支持工作区多文件上传、回收区、恢复/永久删除、成员/管理员权限、审计日志、项目资料异步入库状态和项目 GBrain source 查询；项目对话只读取当前项目、未删除、已同步到当前项目 source 的资料。
...
- Phase 10D/10E 工作区项目资料目录：工作区后端专用目录只允许在系统创建的项目/客户工作区根目录内操作，不允许用户绑定任意本机目录。当前项目资料根路径为 `workspace_data/project/{品牌}/{项目代号}`；`workspace_data/user/{用户名}` 如仍存在，只是会话材料暂存/兼容路径，不再作为个人文件面板产品线。项目侧已补项目 GBrain source、异步录入队列、复杂 PDF/图纸、图片/截图、MP4 自动/长视频分段转写、说话人/术语纠错、EML、EML 附件递归和引用定位 MVP；继续补文件预览、移动功能和更完整的项目质量回归。
