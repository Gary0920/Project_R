...
| 业务 Skill | 会议 Skill 核心闭环已完成，待真实样本与前端手工验收 | 已完成会议文件夹、转录、纪要/行动项、说话人/术语修正、音视频转录、GBrain-ready 生成和后端 E2E；partial、下载体验、真实样本质量回归等仍待补。 |
...
- [ ] 会议工作流只能读取当前工作区中用户有权访问的未删除文件，或用户本轮上传、选择、粘贴的会议资料；不得跨工作区读取会议文件。
- [ ] 用户基于他人上传但自己有权访问的会议文件运行整理时，输出文件的创建者 / 上传者记为本次运行者；审计记录输入文件原上传者和本次运行者。
- [ ] 普通用户只能按现有规则删除自己生成或上传的会议输出文件；项目 / CRM 管理员可治理当前工作区内所有会议文件。
- [x] 个人工作台只允许本轮处理和本地下载，不提供保存到项目 / CRM 或录入 GBrain。
- [ ] 项目 / CRM 文件面板右键提供“会议转录”和“整理会议纪要”入口；Skill 面板提供“项目会议纪要整理 / 行动项提炼”入口；个人工作台只提供轻量处理入口。
- [x] 项目工作区自动创建并使用 `20-会议与沟通/...` 会议资料目录；CRM 工作区使用 `raw/会议记录/...`。
- [x] 按单场会议创建 `YYYYMMDD-HHMM-会议主题` 文件夹，内部使用 `01-原始资料`、`02-转录文本`、`03-辅助总结`、`04-会议纪要`、`05-行动项`。
...
- [ ] 分段转录部分失败时整体状态为 `partial`，已成功片段仍保存；用户可重跑失败片段，不必重跑整个视频。
- [ ] 长视频 / 长音频运行前必须展示预计耗时和高成本模型提示。
- [ ] 长视频、批量会议、PDF / 音视频等高成本处理前必须弹确认；短文本或已有转录文本整理不需要额外成本确认。
- [ ] 转录全部失败时不生成会议纪要或行动项，只显示错误原因和重跑入口。
- [ ] 转录部分失败时允许基于成功片段继续生成纪要和行动项，但 `minutes-vN.md` 必须标记 `转录状态：partial`，列出缺失时间段，并把内容不完整作为风险 / 待确认事项。
- [ ] 失败片段重跑成功后，用户可显式重跑纪要，生成新版本并更新 latest。
- [ ] 第一版转录文本、会议纪要和行动项全部输出 Markdown；DOCX 作为后续正式会议纪要导出能力，不进入第一版主线。
- [ ] 第一版会议类型枚举固定为 `项目统筹会 / 客户沟通会 / 技术交底 / 现场协调 / 内部复盘 / 培训分享 / 其他`。
- [ ] 用户不选择 API Key 或模型；Project_R 后端按文件类型和任务类型自动路由模型，音视频转录走既有 MiMo / 转写链路，纪要整理走文本模型。
- [x] `transcript-latest.md` 必须使用正式会议转录模板，不允许只输出纯文本流水稿。
- [ ] 会议转录模板包含：基本信息、说话人概览、说话人时间轴、疑似术语纠错、完整转录。
- [ ] 完整转录必须包含 `时间点 / 说话人ID / 显示名称 / 内容 / 置信度 / 标记`；说话人可以未确认，但必须有稳定 ID。
- [ ] 说话人概览必须包含 `说话人ID / 显示名称 / 映射状态 / 发言占比 / 发言时长 / 备注`。
- [ ] 疑似术语纠错必须包含 `原识别 / 建议修正 / 类型 / 置信度 / 来源时间点`。
- [ ] 转录结果支持在对话 / 运行结果卡片中下载到本地。
- [ ] 输出说话人占位、发言时间点分布、发言时长或发言占比，支持用户补充说话人映射。
- [ ] 用户修正说话人映射后，默认保存为 `speaker-map-v1.md` 和 `speaker-map-latest.md`，不直接篡改原始转录文本。
- [ ] `speaker-map-latest.md` 必须包含映射状态、修改人、修改时间、说话人映射表和时间轴辅助信息。
- [ ] 会议纪要重跑读取 `transcript-latest.md + speaker-map-latest.md`；只有用户显式选择“应用修正并重写转录显示”时，才生成新版 `transcript-vN.md`。
- [ ] 支持本次运行内术语提示和疑似纠错项记录，不做跨项目自动学习。
- [ ] 说话人映射和术语纠错模板是系统落盘与追溯格式，不是直接要求用户填写的表单。
- [ ] 前端应在对话或运行卡片中用简单问题引导用户，例如“需要标记发言人吗？”“这些词是否需要修正？”；用户跳过时继续运行，并把未确认项标记为待确认。
- [ ] 说话人和术语修正应优先通过轻量交互完成，例如点击说话人、输入姓名、确认/忽略疑似错词，而不是把完整 Markdown 模板暴露给普通用户填写。
- [ ] 会议流程默认先完成自动转录，再基于转录结果展示说话人占比、时间轴和疑似术语错误，然后询问用户是否修正说话人或术语。
- [ ] 用户可跳过说话人或术语修正并继续生成纪要和行动项；跳过后相关负责人、术语和依据不确定项必须标记为待确认。
- [ ] 用户跳过修正后仍保留后补入口；会议文件卡片应提供“编辑说话人”“编辑术语纠错”“应用修正并重跑纪要”等显式动作。
- [ ] 后补修正后不自动覆盖旧纪要；用户显式重跑时生成 `minutes-vN.md`、`actions-vN.md` 并更新 latest，同时写修正和重跑审计。
- [ ] 自动保存写审计，记录操作者、工作区、输入文件、生成文件和 `gbrain_ingest=false`。
- [ ] 长任务完成、失败或 partial 时通知本次运行者，并可点击回到运行卡片；不通知行动项负责人、会议参与人或项目成员全体。
- [ ] 第一版不做行动项提醒、任务到期提醒或跨项目待办通知。
- [ ] 同一会议文件夹内已有 `processing` 运行时，前端禁用重复运行按钮，后端也必须防双击或多用户重复触发。
- [ ] 失败后允许重跑；重新生成新版本必须通过显式重跑动作，不通过并发启动第二个 run。不同会议可以同时运行。
- [ ] 会议工作流生成文件必须出现在当前工作区文件面板中，并用基础状态标签区分 `processing`、`partial`、`ready`、`failed`、`superseded`、`not_ingested`、`ingested`。
- [ ] 第一版文件面板至少要让用户看出哪个是 latest、哪个是旧版本、是否已入库 GBrain、是否失败或部分失败。
- [ ] 已录入 GBrain 的会议资料重跑生成新 latest 后，不自动重新录入；文件面板应标记 latest `needs_reingest` 或等价状态，由有权限用户显式再次录入。

### 9.E.1 会议纪要整理 / 行动项提炼

- [ ] 基于 `transcript-latest.md` 生成会议摘要、决策、行动项、负责人、截止时间、风险和待确认事项。
- [ ] 钉钉或其他会议软件总结只作为辅助材料；关键结论优先回到一手转录文本。
- [ ] 钉钉或其他会议软件总结可自动作为辅助材料参与整理，但必须位于同一会议文件夹的 `03-辅助总结`，或作为同名 sidecar，例如 `meeting.dingtalk-summary.md`、`meeting.summary.md`、`meeting.ai-summary.md`。
- [ ] 右键单文件运行时只查找同名辅助总结 sidecar，不扫描整个目录中的其他总结文件。
- [ ] 只来自辅助总结、无法在转录文本中找到依据的事实、决策或行动项，必须标记为辅助总结来源或待确认。
- [ ] 未确认说话人映射产生的负责人标记为待确认。
- [ ] `minutes-latest.md` 必须使用正式会议纪要模板，不允许只输出简化摘要。
- [ ] 会议纪要模板包含：会议基本信息、一句话结论、按议题组织的会议摘要、关键决策、行动项、风险与问题、待确认事项、资料与证据、生成说明。
- [ ] 关键决策表必须包含 `ID / 决策 / 决策背景 / 影响范围 / 来源时间点 / 依据摘录 / 置信度 / 待确认`。
- [ ] 行动项表必须包含 `ID / 行动项 / 负责人 / 协作人 / 截止时间 / 优先级 / 状态 / 来源时间点 / 待确认`，状态第一版固定为 `待确认 / 待执行 / 已完成 / 已取消`。
- [ ] 风险与问题表必须包含 `ID / 风险或问题 / 类型 / 影响 / 建议下一步 / 负责人 / 来源时间点 / 严重度`，类型至少支持 `技术 / 工期 / 成本 / 商务 / 客户 / 资料缺口`。
- [ ] 待确认事项表必须包含 `ID / 待确认事项 / 为什么需要确认 / 建议确认对象 / 来源时间点`。
- [ ] 资料与证据表必须区分一手转录、辅助总结、用户补充和原始音视频；未能在转录中找到依据的内容必须标记为待确认或注明来源为整理材料。
- [ ] 会议纪要可包含 `可沉淀知识候选`，按公司规则候选、项目经验候选、流程改进候选、模板 / 检查清单候选分类；第一版只保存在会议纪要中，不自动提交公司知识审核或写入 `company-wiki`。
- [ ] `actions-latest.md` 必须使用正式行动项跟进模板，不允许只复制会议纪要中的行动项表。
- [ ] 行动项跟进模板包含：基本信息、行动项总览、行动项清单、按负责人分组、按截止时间排序、待确认行动项、生成说明。
- [ ] 行动项清单必须包含 `ID / 状态 / 优先级 / 行动项 / 负责人 / 协作人 / 截止时间 / 依赖条件 / 来源时间点 / 依据摘录 / 待确认原因`。
- [x] 没有明确负责人、截止时间或行动描述的行动项不得编造；必须标记为 `待确认` 并写明待确认原因。
- [ ] 第一版行动项只生成结构化 Markdown 文件和运行结果摘要，不创建 Project_R 独立任务对象，不做负责人账号绑定、提醒、任务看板或日历。
- [x] 生成 `minutes-v1.md`、`actions-v1.md` 和对应 latest 文件；重跑生成新版本，不覆盖旧版本。
- [ ] 用户修正说话人映射后，不自动重跑；提供显式“应用说话人修正并重跑纪要”动作。
- [ ] 自动保存前展示目标文件夹和预计文件清单；保存后展示摘要、文件链接、下载按钮和重跑入口。
- [ ] 第一版不做完整 Markdown 在线编辑器；用户可下载、打开文件或通过显式重跑生成新版本，后续再考虑轻量修正指令。
- [ ] 第一版不支持任意自然语言局部修改已生成纪要或行动项；只支持说话人映射修正、术语纠错和显式重跑。遇到不可自动修改的请求时，通知用户当前需手动调整文件。
- [x] 保存不等于自动入库 GBrain，仍写 SkillRun / 会议处理 run 审计。

### 9.E.2 会议资料录入 GBrain 适配

- [x] 项目工作区会议资料录入当前项目 source；CRM 工作区会议资料录入 CRM 客户情报 source。
- [ ] 会议资料归属由文件所在工作区和用户触发录入的位置决定，不由会议内容自动判断。
- [ ] 录入确认弹窗显示当前工作区、目标 source、路径、递归范围、数量、类型和高成本模型提示。
- [x] 文件夹批量录入按单场会议聚合，只默认吸收 latest 或最高版本文件。
- [x] `v1`、`v2` 等旧版本默认跳过，在 manifest 标记 `skipped_superseded_version`；用户可右键具体旧版本单独录入。
- [ ] 右键“录入此文件”只处理当前文件，可查找同名 sidecar，不扫描整个会议目录。
- [ ] 原始音视频不直接作为 GBrain 正文；录入音视频时先转录，再基于转录文本生成 GBrain-ready 会议资料。
- [ ] 人工整理纪要可入库但必须标记为整理结果，不能伪装成一手转录。
- [x] 会议资料默认录入组合为 `minutes-latest.md + transcript-latest.md`；`actions-latest.md` 作为会议 GBrain-ready 页面中的结构化行动项辅助，不默认生成独立知识页。
- [ ] GBrain-ready 会议页面应包含会议摘要、决策、行动项、风险、待确认事项、证据摘录和指向一手转录的来源引用。
- [ ] 用户右键单独录入 `actions-latest.md` 时允许处理，但 GBrain-ready 页面必须标记 `source_context: action_items_only` 或等价低上下文状态，并提示建议录入完整会议资料。
- [ ] 如果同会议文件夹存在 `minutes-latest.md` 和 `transcript-latest.md`，单文件行动项录入确认弹窗应建议改为录入完整会议；除非用户选择文件夹录入，否则不得自动吞入同文件夹其他文件。

### 9.E.3 第一版质量验收与不做项

- [ ] 第一版质量验收至少覆盖短文本会议、已有转录 DOCX / MD、MP4 长视频三类样本，并分别验证转录、纪要、行动项、自动保存、下载、重跑和审计。
- [ ] 第一版明确不做 DOCX 导出、完整在线编辑器、独立任务系统、自动提交 company-wiki、跨会议说话人记忆、全局术语学习和自动重新入库 GBrain。
...
- 后端 E2E 测试已覆盖完整工作流（36 passed），但真实样本验收（短文本/DOCX/MP4 三类）和前端手工验收仍待执行。

### 9.E 会议 Skill 当前进度 (2026-06-09)
...
| 7 | 端到端验收与体验收口 | ✅ E2E 测试 + 文档 | 后端 E2E 完成，前端手工验收待补 |
| 8 | 真实样本质量验收 | 待启动 | 短文本/DOCX/MP4 三类真实样本 |
