...
function getRandomWord(prev?: string): string {
  ...
  function renderMessageCard(message: ChatMessage) {
    ...
    return (
      <article className={`message-row message-row-${message.role} ${message.status === "failed" ? "message-row-failed" : ""}`} key={message.id} data-turn={message.id}>
    ...
    return (
      ...
      >
        ...
        <div className="message-scroll-wrap">
          ...
          {isActivePane ? <JumpBar messages={paneMessages} scrollRef={scrollRef} /> : null}
        </div>
